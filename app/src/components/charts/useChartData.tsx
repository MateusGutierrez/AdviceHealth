...
export const useChartData = () => {
    ...
    const weekSeries = [
      ...
      const doctorSpecialtiesCount = doctorsSchedule.reduce((acc: Record<string, number>, doctor) => {
        acc[doctor.specialty] = (acc[doctor.specialty] || 0) + 1;
        return acc;
      }, {});
    
      const doctorSeries = Object.entries(doctorSpecialtiesCount).map(([specialty, count]) => ({
        label: specialty,
        value: count,
      }));
