import {
  EditScheduleFormValue,
  EditScheduleSchema,
} from '../../schemas/EditScheduleSchema';
import {
  Button,
  FormControl,
  Input,
...
const EditScheduleForm = ({
  ...
  const { doctorsSchedule, date, editSchedule, submitSchedule } = useStore(
    (state) => state
  );
    ...
    () =>
      doctor && date
        ? head(filter(doctorsSchedule, (d) => d.doctorName === doctor))
            ?.weeklyAvailableSlots[formatedDate] || []
        : [],
