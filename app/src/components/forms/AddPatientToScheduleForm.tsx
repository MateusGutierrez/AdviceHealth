...
const AddPatientToScheduleForm = ({ hide }: Props) => {
  ...
  const { doctorsSchedule, addPatientData, submitSchedule, addToSchedule } =
  ...
  const onSubmit = useCallback(
    (data: AddPatientToScheduleFormValue) => {
  ...
  const availableTimes = useMemo(() => {
    return doctor && selectedDay
      ? head(filter(doctorsSchedule, (d) => d.doctorName === doctor))
          ?.weeklyAvailableSlots[selectedDay] || []
      : [];
  ...
  return (
    <StyledForm onSubmit={handleSubmit(onSubmit)}>
      <div id="header">
        ...
        <Button onClick={hide}>
          <i className="bi bi-x" id="x" onClick={hide} />
        </Button>
      </div>
      ...
      <Button
        type="submit"
        disabled={!selectedTime || !isEmpty(errors)}
