...
const isValidCPF = (cpf: string): boolean => {
  ...
  if (sanitizedCPF.length !== 11 || /^(\d)\1+$/.test(sanitizedCPF)) return false;

  const calculateDigit = (factor: number): number => {
    const total = sanitizedCPF
      .slice(0, factor - 1)
      .split("")
      .reduce((sum, num, index) => sum + parseInt(num, 10) * (factor - index), 0);
  ...
  return digit1 === parseInt(sanitizedCPF[9], 10) && digit2 === parseInt(sanitizedCPF[10], 10);
};
...
export type AddPatientToScheduleFormValue = z.infer<typeof AddPatientToScheduleSchema>;